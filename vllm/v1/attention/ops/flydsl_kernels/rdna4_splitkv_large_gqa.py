...
def compile_large_gqa_stage(
    *,
    ...
    if not 9 <= query_group_size <= 2 * NUM_WAVES:
        raise ValueError(
            f"query_group_size must be between 9 and {2 * NUM_WAVES}, "
    ...
    # The FP8 path materializes K/V directly in WMMA fragments, so its LDS
    # arena only needs the phase-reused score/probability storage plus Q.  Do
    # not reserve the BF16 K/V staging footprint for that specialization.
    if is_fp8:
        large_score_offset = 0
        large_prob_offset = 0
        large_alpha_offset = PROB_BYTES
        large_q_offset = max(SCORE_BYTES, large_alpha_offset + ALPHA_BYTES) + 2048
        large_lds_bytes = large_q_offset + Q_BYTES
    else:
        large_score_offset = SCORE_OFFSET
        large_prob_offset = PROB_OFFSET
        large_alpha_offset = ALPHA_OFFSET
        large_q_offset = Q_OFFSET
        large_lds_bytes = LDS_BYTES

    @fx.struct
    class SharedStorage:
        words: fx.Array[fx.Int32, large_lds_bytes // 4, 16]
    ...
    def grouped_stage1(
        ...
        if const_expr(is_fp8):
            ...
            prefetch_value_u8x8, consume_value_u8x8 = make_64b_prefetcher(
                value_ptr, fx.Uint8, 8, 2, recast=True
            )
        ...
        s_query = shared_view(
            large_q_offset, fx.BFloat16, (16, HEAD_DIM), (HEAD_DIM, 1)
        )
        s_key = shared_view(0, fx.BFloat16, (TILE_TOKENS, HEAD_DIM), (HEAD_DIM, 1))
        s_score = shared_view(
            large_score_offset, fx.Float32, (16, TILE_TOKENS), (TILE_TOKENS, 1)
        )
        s_value = shared_view(0, fx.BFloat16, (HEAD_DIM, TILE_TOKENS), (TILE_TOKENS, 1))
        s_prob = shared_view(
            large_prob_offset, fx.BFloat16, (16, TILE_TOKENS), (TILE_TOKENS, 1)
        )
        s_alpha = shared_view(large_alpha_offset, fx.Float32, (16,), (1,))
        ...
        init_state = [neg_inf, zero, neg_inf, zero] + [
            zero for _ in range_constexpr(16)
        ]
        ...
        for tile, state in range(begin, end, TILE_TOKENS, init=init_state):
            ...
            accum = [fx.Float32(state[4 + j]) for j in range_constexpr(16)]
            ...
            for row_slot in range_constexpr(2):
                ...
                if live_row:
                    s_prob[owned_row, token0] = p0.to(fx.BFloat16)
                    s_prob[owned_row, token1] = p1.to(fx.BFloat16)
                    if lane == 0:
        ...
        for out_repeat in range_constexpr(2):
            d = wave * 32 + out_repeat * 16 + lane % 16
