...
"""Grouped-query WMMA stage for RDNA4 SplitKV paged attention."""
...
# Q[16,256] remains live across all context tiles. The lower arena is reused:
# phase 1 holds K[64,256] + score[16,64], and phase 2 holds
# V[256,64] + P[16,64] + six FP32 online-softmax rescale factors.
...
ALPHA_BYTES = 8 * 4
# Keep a 2 KiB guard after the phase-reused output tile. The WMMA C-copy
# layout is exact logically, but its lowered vector transactions use a padded
# physical footprint; placing the persistent Q tile at the nominal end makes
# correctness depend on CTA scheduling/specialization.
Q_OFFSET = max(SCORE_OFFSET + SCORE_BYTES, ALPHA_OFFSET + ALPHA_BYTES) + 2048
LDS_BYTES = Q_OFFSET + Q_BYTES


@functools.lru_cache(maxsize=128)
def compile_grouped_stage(
    *,
    kv_dtype: str,
    splits: int,
    num_kv_heads: int,
    ...
    if kv_dtype not in ("bf16", "fp8"):
        raise ValueError(f"kv_dtype must be 'bf16' or 'fp8', got {kv_dtype!r}")
    if num_kv_heads < 1:
        raise ValueError(f"num_kv_heads must be positive, got {num_kv_heads}")
    if not 1 <= query_group_size < NUM_WAVES:
        raise ValueError(
            f"query_group_size must be between 1 and {NUM_WAVES - 1}, "
    ...
    def grouped_stage1(
        ...
        def dequant_fp8x8(raw, scale):
            return _dequant_fp8x8(raw, scale, fx.BFloat16, is_fp8fnuz=False)
        ...
        s_alpha = shared_view(ALPHA_OFFSET, fx.Float32, (8,), (1,))
        ...
        for q_iter in range_constexpr(16):
            ...
            s_query[q_local, d] = valid_q.select(q_value, fx.BFloat16(0.0))
        lds_barrier()

        neg_inf = fx.Float32(float("-inf"))
        zero = fx.Float32(0.0)
        init_state = [neg_inf, zero] + [zero for _ in range_constexpr(16)]
        ...
        for tile, state in range(begin, end, TILE_TOKENS, init=init_state):
            ...
            accum = [fx.Float32(state[2 + j]) for j in range_constexpr(16)]
            ...
            # One live query row per wave. Waves 6/7 operate on padded rows and
            # are masked before publishing probabilities.
            live_row = wave < query_group_size
            row = live_row.select(wave, fx.Int32(0))
            token0 = lane
            token1 = lane + WAVE_SIZE
            valid0 = fx.Int32(tile) + token0 < end
            valid1 = fx.Int32(tile) + token1 < end
            score0 = valid0.select(
                fx.Float32(s_score[row, token0]) * softmax_scale, neg_inf
            )
            score1 = valid1.select(
                fx.Float32(s_score[row, token1]) * softmax_scale, neg_inf
            )
            local_max = fx.max(score0, score1)
            tile_max = _wave_reduce(local_max, "max")
            new_max = fx.max(running_max, tile_max)
            p0 = valid0.select(fmath.exp2((score0 - new_max) * LOG2E), zero)
            p1 = valid1.select(fmath.exp2((score1 - new_max) * LOG2E), zero)
            tile_sum = _wave_reduce(p0 + p1, "sum")
            alpha = fmath.exp2((running_max - new_max) * LOG2E)

            # Clear all padded rows, then publish the six live probability rows.
            for p_iter in range_constexpr((16 * TILE_TOKENS) // BLOCK_THREADS):
                linear = tid + p_iter * BLOCK_THREADS
                s_prob[linear // TILE_TOKENS, linear % TILE_TOKENS] = fx.BFloat16(0.0)
            if live_row:
                s_prob[row, token0] = p0.to(fx.BFloat16)
                s_prob[row, token1] = p1.to(fx.BFloat16)
                if lane == 0:
                    store_alpha(row, alpha)
            ...
            value_wave_ptr = fx.add_offset(
                fx.get_iter(s_value), wave * 32 * TILE_TOKENS
            )
            value_wave = fx.make_view(
                value_wave_ptr,
                fx.make_layout((32, TILE_TOKENS), (TILE_TOKENS, 1)),
            )
            ...
            if const_expr(is_fp8):
                ...
                fragment_values = []
                # The fragment's register layout is (value, N-repeat,
                # K-repeat). A whole-fragment store therefore expects K, N,
                # value iteration order; storing individual repeated slices
                # loses the N-repeat offset during lowering.
                for token_fragment in range_constexpr(TILE_TOKENS // 16):
                    ...
                    for column_fragment in range_constexpr(2):
                        d = wave * 32 + column_fragment * 16 + lane_column
                        value_index = (
                            physical_page * v_stride0
                            + kv_head * v_stride1
                            + d * v_stride2
                        ...
                        prefetch_value_u8x8(
                            token_fragment * 2 + column_fragment,
                            value_index,
                        )
                fx.rocdl.s_wait_loadcnt(0)
                for token_fragment in range_constexpr(TILE_TOKENS // 16):
                    ...
                    for column_fragment in range_constexpr(2):
                        value_values = dequant_fp8x8(
                            consume_value_u8x8(token_fragment * 2 + column_fragment),
                            v_scale,
                        )
                ...
                frag_v.store(
                    fx.Vector.from_elements(fragment_values, dtype=fx.BFloat16)
                )
            else:
            ...
            lds_barrier()
            results = yield [new_max, running_sum * alpha + tile_sum] + next_accum

        has_tokens = end > begin
        if wave < query_group_size:
            qh = kv_head * query_group_size + wave
            final_sum = fx.Float32(results[1])
            if lane == 0:
                store_alpha(wave, final_sum)
        ...
        if lane < 16:
            for out_repeat in range_constexpr(2):
                ...
                for output_row in range_constexpr(query_group_size):
                    ...
                    accum_index = 2 + out_repeat * 8 + output_row
