...
"""Native BF16 RDNA4 SplitKV stage."""
...
def compile_native_bf16_d256_stage(
    *,
    splits: int,
    num_kv_heads: int,
    ...
    assert lds_bytes == NATIVE_BF16_LDS_BYTES
    ...
    def grouped_stage1(
        ...
        if const_expr(is_fp8):
            ...
            prefetch_value_u8x8, consume_value_u8x8 = make_64b_prefetcher(
                value_ptr, fx.Uint8, 8, 4 * output_repeats, recast=True
            )
        if const_expr(direct_cache and not is_fp8):
            load_key_bf16x8 = make_128b_loader(key_ptr, fx.BFloat16, 8)
            prefetch_value_bf16x8, consume_value_bf16x8 = make_128b_prefetcher(
                value_ptr, fx.BFloat16, 8, 4 * output_repeats
            )
        ...
        def load_key_fragment(tile_base, token_wave):
            ...
            for d_fragment in range_constexpr(head_dim // 16):
                ...
                if const_expr(is_fp8):
                    ...
                    key_values = load_key_bf16x8(key_index)
        ...
        for tile, state in range(begin, end, TILE_TOKENS, init=init_state):
            ...
            if const_expr(direct_cache):
                ...
                if const_expr(is_fp8):
                    ...
                    # Leave the last two FP16 V-fragments in flight.  The
                    # compiler inserts per-fragment dependency waits as each
                    # WMMA operand is consumed.
                ...
                for token_fragment in range_constexpr(TILE_TOKENS // 16):
                    ...
                    for column_fragment in range_constexpr(output_repeats):
                        if const_expr(is_fp8):
                            ...
                            value_values = consume_value_bf16x8(
                                token_fragment * output_repeats + column_fragment
                            )
