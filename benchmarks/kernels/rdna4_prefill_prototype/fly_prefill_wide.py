...
def compile_stage(batch, hq, hk, dim, page, qcap, splits, strides):
    ...
    @fx.struct
    class Shared:
        scores: fx.Array[fx.Float32, 32 * 68, 16]
        weights: fx.Array[fx.BFloat16, 32 * 72, 16]
        query: fx.Array[fx.BFloat16, 32 * (dim + 8), 16]
        rescale: fx.Array[fx.Float32, 32, 16]
        denom: fx.Array[fx.Float32, 32, 16]
    ...
    def stage(
        ...
        if (nq > 0) & (nq <= 128) & (mt * 32 < nq * g):
            ...
            scores = storage.scores.view(fx.make_layout((32, 64), (68, 1)))
            weights = storage.weights.view(fx.make_layout((32, 64), (72, 1)))
            scales = storage.rescale.view(fx.make_layout(32, 1))
            denoms = storage.denom.view(fx.make_layout(32, 1))
            ...
            qt = storage.query.view(fx.make_layout(32 * (dim + 8), 1))
            qtdiv = fx.logical_divide(qt, fx.make_layout(1, 1))
            qcopy = fx.make_rmem_tensor(fx.make_layout(8, 1), fx.BFloat16)
            for it in range_constexpr(32 * dim // (128 * 8)):
                word = tid * 8 + it * 128 * 8
                row = word // dim
                dc = word % dim
                qr = mt * 32 + row
                ...
                qh = kh * g + qr % g
                qi = qtoken * q0 + qh * q1 + dc
                fx.copy(copy, fx.slice(qdiv, (None, qi)), qcopy)
                fx.copy(copy, qcopy, fx.slice(qtdiv, (None, row * (dim + 8) + dc)))
            ...
            init = [neg, zero] * 8 + [zero for _ in range_constexpr(cols * 16)]
            for tile, state in range(fx.Int32(0), tiles, fx.Int32(1), init=init):
                ...
                for jbase in range_constexpr(dim // 64):
                    ...
                    for rt in range_constexpr(2):
                        for jj in range_constexpr(4):
                            ...
                            fx.copy(copy, fx.slice(qtdiv, (None, qi)), qfrags[rt][jj])
                        for jj in range_constexpr(4):
                            fx.mma_atom_call(
                                mma, sf[rt], qfrags[rt][jj], kfrags[jj], sf[rt]
                            )
                for rt in range_constexpr(2):
                    sv = fx.Vector(sf[rt].load())
                    for r in range_constexpr(8):
                        scores[
                            rt * 16 + (lane // 16) * 8 + r, wave * 16 + lane % 16
                        ] = sv[r] * scale
                fx.rocdl.s_waitcnt(lgkmcnt=0)
                gpu.barrier()
                stats = []
                for rr in range_constexpr(8):
                    row = wave + rr * 4
                    m = mt * 32 + row
                    a = base + lane
                    b = a + 32
                ...
                for rt in range_constexpr(2):
                    for j in range_constexpr(4):
                        for e in range_constexpr(8):
                            pfrags[rt][j][e] = weights[
                                rt * 16 + lane % 16, j * 16 + (lane // 16) * 8 + e
                            ]
                rscales = [
                    [
                        fx.Float32(scales[rt * 16 + (lane // 16) * 8 + r])
                        for r in range_constexpr(8)
                    ]
                    for rt in range_constexpr(2)
                ]
                ...
                for c in range_constexpr(cols):
                    ...
                    for rt in range_constexpr(2):
                        for r in range_constexpr(8):
                            of[rt][c][r] = (
                                fx.Float32(state[16 + (rt * cols + c) * 8 + r])
                                * rscales[rt][r]
                            )
                        for j in range_constexpr(4):
                            fx.mma_atom_call(
                                mma, of[rt][c], pfrags[rt][j], vfrags[j], of[rt][c]
                            )
                for rt in range_constexpr(2):
                    for c in range_constexpr(cols):
                        oval = fx.Vector(of[rt][c].load())
            ...
            for rt in range_constexpr(2):
                for c in range_constexpr(cols):
                    dc = wave * (dim // 4) + c * 16 + lane % 16
                    for r in range_constexpr(8):
                        row = rt * 16 + (lane // 16) * 8 + r
                        m = mt * 32 + row
                        if m < nq * g:
                            den = fx.Float32(denoms[row])
                            safe_den = (den > 0.0).select(den, fx.Float32(1.0))
                            pi = (
                                ((split * batch + seq) * hk + kh) * rows + m
                            ) * dim + dc
                            part[pi] = (
                                fx.Float32(results[16 + (rt * cols + c) * 8 + r])
                                / safe_den
                            )
    ...
    def launch(
        ...
        stage(Q, K, V, DK, DV, T, START, LEN, P, L).launch(
            grid=(((rows + 31) // 32) * batch * hk * splits,),
            block=(128,),
            stream=stream,
        )
